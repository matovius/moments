...
export const createJournalSchema = z
	...
	.refine(({ title, description, currentDate }) => {
		return title || description || currentDate;
	}, 'At least one field must be filled out');

export type CreateJournalSchema = typeof createJournalSchema;

export const deleteJournalSchema = z.object({
	id: z.string()
});

export type DeleteJournalSchema = typeof deleteJournalSchema;
